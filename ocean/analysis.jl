...
# Spatial resolution of initial conditions
res = 0.05
...
@showprogress desc = "Computing matrices for error model..." for (i, x0) in
																 enumerate(inits)
...
fig.savefig("$fdir/S2_field_error.pdf"; dpi = dpi, bbox_inches = "tight")
close(fig)
...
ylim(lat_range)
ax.set_xticks(ax.get_xticks(), -Int64.(ax.get_xticks()))
fig.savefig("$fdir/s2min_field_error.pdf"; dpi = dpi, bbox_inches = "tight")
close(fig)
...
ylim(lat_range)
ax.set_xticks(ax.get_xticks(), -Int64.(ax.get_xticks()))
fig.savefig("$fdir/cov_field_error.pdf"; dpi = dpi, bbox_inches = "tight")
close(fig)
...
ylim(lat_range)
ax.set_xticks(ax.get_xticks(), -Int64.(ax.get_xticks()))
fig.savefig("$fdir/ratio_field_error.pdf"; dpi = dpi, bbox_inches = "tight")
close(fig)
...
ylim(lat_range)
ax.set_xticks(ax.get_xticks(), -Int64.(ax.get_xticks()))
fig.savefig("$fdir/ftle_field_error.pdf"; dpi = dpi, bbox_inches = "tight")
close(fig)
...
for R in [0.25, 0.5, 1.0, 2.0, 4.0, 6.0]
	...
	fig.savefig("$fdir/S2_robust_error_$R.pdf"; dpi = dpi, bbox_inches = "tight")
	close(fig)
end

###################################### EDDY PARAMETERISATION #######################################
# Parameters
eddyT = 3.8  # Eddy lifetime - days
eddyS_ms = 1800    # Eddy diffusivity - m/s
...
function σσᵀ_eddy!(s, x, t)
	...
	s[3, 3] = 2 / eddyT * arc_to_meridonal(WGS84, x[2]) * eddyS_ms
	s[4, 4] = 2 / eddyT * arc_to_meridonal(WGS84, x[2]) * eddyS_ms
end

# Can just reuse wtmp, Σtmp, inits, S2 from previous model
@showprogress desc = "Computing matrices for eddy parameterisation..." for (i, x0) in enumerate(inits)
