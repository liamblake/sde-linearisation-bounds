...
"""
function generate_or_load!(generate::Bool, y_rels, z_rels, gauss_z_rels, gauss_y_rels, model, space_time, N, εs, data_fname::String)
    if generate
        # Solve the SDE to generate new data
        generate_data!(y_rels, z_rels, gauss_z_rels, gauss_y_rels, model, space_time, N, εs)
...
GENERATE = false
generate_or_load!(GENERATE, y_rels, z_rels, gauss_z_rels, gauss_y_rels, model, space_time, N, εs, data_fname)
...
theorem_validation(y_rels, z_rels, gauss_z_rels, gauss_y_rels, model, space_time, εs, rs, dt)
...
xs = 0:0.005:π
ys = 0:0.005:π
x₀_grid = [collect(pairs) for pairs in Base.product(xs, ys)][:]
